// Package sourcearchive creates in-memory tar archives of project
// directories for BuildRequest submissions, applying caller-supplied
// exclusion rules.
...
// ExcludeFunc reports whether the file or directory at relPath
// (slash-separated, relative to the project root) should be left out of
// the archive. Returning true for a directory skips the whole subtree.
type ExcludeFunc func(relPath string, isDir bool) bool

// Create archives projectPath into an in-memory tar and returns the
// archive bytes plus its SHA-256 hex checksum.
func Create(projectPath string, exclude ExcludeFunc) ([]byte, string, error) {
