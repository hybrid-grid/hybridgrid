...
Bước 4--5 trong Hình~\ref{fig:lifecycle} phải chịu được nhiều lời gọi \texttt{Compile()} đồng thời: với cờ \texttt{make -jN}, có tới $N$ tác vụ cùng lúc yêu cầu một quyết định. Quyết định lập lịch đọc trạng thái worker (\texttt{ActiveTasks}) và việc dispatch ghi lại trạng thái đó; nếu hai bước là hai thao tác tách rời giữa các goroutine đồng thời, hai dispatch có thể cùng đọc một giá trị \texttt{ActiveTasks} cũ, cùng chọn một worker năng lực thấp và gây overbook.
...
\textbf{Cập nhật nghịch đảo Sherman--Morrison.} Mỗi cập nhật là hạng-1: $A_{\text{new}}=A_{\text{old}}+xx^\top$. Nghịch đảo lại từ đầu tốn $O(d^3)$; công thức Sherman--Morrison~\cite{sherman1950} ở dòng 15 giảm còn $O(d^2)$:
\begin{equation}
...
Bản thân độ lệch chưa nói được scheduler có dồn việc vào \emph{đúng chỗ} hay không, nên chúng tôi đối chiếu từng phân phối dispatch với phân phối mà năng lực của cụm hàm ý. Năm worker nắm 12,5\,/\,15,0\,/\,20,0\,/\,25,0\,/\,27,5\% trong tổng 4.000 milli-core của cụm, và một scheduler phân bổ đúng tỉ lệ năng lực sẽ tái hiện chính xác các tỉ lệ đó. Đây không trùng với bố cục slot, vốn là 10\,/\,20\,/\,20\,/\,20\,/\,30\% (\texttt{MaxParallel} bằng 1, 2, 2, 2, 3) - nên chia đều theo worker hay chia đều theo slot đều không phải là chia theo năng lực.
