...
Bộ lập lịch thứ sáu, \textbf{icecc-fastest}, cũng được cài đặt sau cùng giao diện trên. Đây là port Go trung thực cho luật chọn \texttt{fastest} của icecream (icecc) --- trình biên dịch phân tán C/C++ mã nguồn mở dựa trên include-what-you-use, triển khai rộng rãi trên các cụm build sản xuất --- được viết lại native bên trong Hybrid-Grid thay vì chạy đầu-cuối, sao cho mọi khác biệt makespan đều quy về được luật quyết định mà không lẫn khác biệt về transport, nén hay vận chuyển toolchain. Hàng \texttt{icecc-fastest} xuất hiện trong các bảng kết quả do pipeline phân tích (\texttt{scripts/analyze\_rigorous.py}) sinh ra; nó không được đưa vào quy trình đánh giá chặn khối ở Mục~\ref{sec:eval} và không mang phát biểu nào có hậu thuẫn thống kê trong bài báo này. Chúng tôi nêu tên ở đây chỉ để đảm bảo mọi định danh scheduler có mặt trong dữ liệu công bố đều được liệt kê trong phần tường thuật của bài.
...
\textbf{Nguồn số liệu throughput.} Mọi thời gian theo từng tác vụ và các con số throughput \texttt{-j5} báo cáo trong bài được trích từ task log JSONL của coordinator (Mục~\ref{sec:log}): mỗi dòng là một bản ghi cấp tác vụ do \texttt{TaskLogger} phát ra khi lời gọi \texttt{Compile()} hoàn tất, mang dấu thời điểm dispatch, ngữ cảnh worker, và phân rã độ trễ đầy đủ. Throughput do đó được đếm là số bản ghi tác vụ hoàn tất, chứ không phải bằng cách phân tích console output hay \texttt{wc -l} trên log build --- cách sau trộn lẫn một tác vụ với nhiều dòng output verbose của trình biên dịch và phụ thuộc mức ghi log. Task log JSONL là nguồn duy nhất, tái lập được: nạp vào pandas bằng \texttt{read\_json(lines=True)} cho ra chính xác các phép đo theo từng tác vụ nuôi mọi bảng và phân tích cơ chế dưới đây.

\textbf{Giữ riêng hai dataset \texttt{-j5}.} Hai bộ đo \texttt{-j5} độc lập được thu thập dưới điều kiện chạy hơi khác nhau (session khác nhau, cách nhau về thời gian). Chúng được báo cáo và phân tích \textbf{riêng biệt} xuyên suốt bài báo và không bao giờ gộp hay pool: phép so sánh chặn khối ở Mục~\ref{sec:j5} (Bảng~\ref{tab:j5}) và phép kiểm chứng warm-bandit ở Mục~\ref{sec:coldstart} mỗi cái rút từ bộ của riêng nó, và không có kết quả tổng hợp nào kết hợp bản ghi qua hai bộ. Lựa chọn bảo thủ này --- giữ $n$ ở giá trị mỗi bộ thay vì phồng trọng số thống kê bằng cách pool --- được thực hiện để bạn đọc có thể đối chiếu hai bộ song song mà không trộn lẫn hiệu ứng session.
...
\section*{Tuyên bố về Generative AI và công nghệ hỗ trợ AI trong quá trình viết}
