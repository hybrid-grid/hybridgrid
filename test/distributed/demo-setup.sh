...
GOOS=darwin GOARCH=arm64 go build -ldflags "$LDFLAGS" -o "$OUTPUT_DIR/mac/hg-dashboard" ./cmd/hg-dashboard
...
echo "│   ├── hg-dashboard  (optional standalone dashboard)"
...
echo ""
echo "2. START DASHBOARD (this Mac, optional):"
echo "   $OUTPUT_DIR/mac/hg-dashboard serve --port=8081 --coordinator=localhost:9000 --insecure"
echo "   # Open: http://localhost:8081"
echo ""
echo "3. COPY & START WORKER on Raspberry Pi 5:"
echo "   scp $OUTPUT_DIR/raspi/hg-worker pi@<RASPI_IP>:~/"
echo "   ssh pi@<RASPI_IP> './hg-worker serve --coordinator=<MAC_IP>:9000'"
echo ""
echo "4. COPY & START WORKER on Windows PC:"
echo ""
echo "5. CLONE TEST PROJECT (on this Mac):"
...
echo ""
echo "7. DASHBOARD: http://localhost:8081"
